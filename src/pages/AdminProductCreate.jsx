...
import { clearAdminProductMessages, createProduct } from '../redux/adminProductSlice';
import api from '../services/api';

const AdminProductCreate = () => {
  const dispatch = useDispatch();
  const navigate = useNavigate();
  const { loading, error, success } = useSelector((s) => s.adminProducts);
  ...
  const errors = useMemo(() => {
    ...
    if (!images.length) next.images = 'At least one image is required.';
    return next;
  }, [form, images]);
  ...
  const onImagesChange = (e) => {
    ...
    const maxFiles = 8;
    const merged = [...images, ...files].slice(0, maxFiles);
    setImages(merged);
          ...
          <header className="flex flex-col gap-3 border-b border-slate-200 pb-4 sm:flex-row sm:items-center sm:justify-between dark:border-slate-800">
            <div>
              ...
              <p className="text-xs text-slate-600 sm:text-sm dark:text-slate-400">
                Add a new product to ShopEZ, including images, pricing, and variants.
              </p>
            </div>
          </header>
              ...
              <div className="space-y-4">
                ...
                <div>
                  ...
                  <textarea
                    ...
                    onChange={onChange}
                    rows={4}
                    placeholder="High-level description for the product detail page."
                    className="mt-1 w-full rounded-xl border border-slate-300 bg-slate-50 px-3 py-2.5 text-sm text-slate-900 placeholder:text-slate-500 outline-none focus:ring-2 focus:ring-primary-500/40 dark:border-slate-800 dark:bg-slate-950/40 dark:text-slate-50 dark:placeholder:text-slate-600"
                  />
                </div>
                  ...
                  <div>
                    ...
                    {categories.length === 0 && (
                      <p className="mt-1 text-[0.65rem] text-slate-500 dark:text-slate-400">
                        No categories found yet. Type a category name to create one on the fly.
                      </p>
                    )}
                  </div>
                  <div>
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <p className="mt-0.5 text-[0.65rem] text-slate-500 dark:text-slate-400">
                    Select multiple photos (up to 8). First image is the main thumbnail.
                  </p>
                  <input
                    type="file"
                    multiple
                    ...
                    className="mt-2 block w-full text-[0.75rem] text-slate-700 file:mr-3 file:rounded-lg file:border-0 file:bg-slate-100 file:px-3 file:py-1.5 file:text-[0.75rem] file:font-medium file:text-slate-800 hover:file:bg-slate-200 dark:text-slate-200 dark:file:bg-slate-800 dark:file:text-slate-100 dark:hover:file:bg-slate-700"
                  />
                  {touched.images && errors.images && (
                    <p className="mt-1 text-xs text-red-500 dark:text-red-300">{errors.images}</p>
                  )}
                  ...
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <p className="text-[0.7rem] text-slate-500 dark:text-slate-400">
                        Track size / color / storage with per-variant stock.
                      </p>
                    </div>
                    <button
                      type="button"
                  ...
                  <div className="mt-2 space-y-3">
                    <div>
                      ...
                      <input
                        ...
                        onChange={onChange}
                        placeholder="Title for search engines and social previews."
                        className="mt-1 w-full rounded-xl border border-slate-300 bg-slate-50 px-3 py-2.5 text-sm text-slate-900 placeholder:text-slate-500 outline-none focus:ring-2 focus:ring-primary-500/40 dark:border-slate-800 dark:bg-slate-950/40 dark:text-slate-50 dark:placeholder:text-slate-600"
                      />
                    </div>
                    <div>
                      ...
                      <textarea
                        ...
                        onChange={onChange}
                        rows={2}
                        placeholder="Short, search-friendly summary of the product."
                        className="mt-1 w-full rounded-xl border border-slate-300 bg-slate-50 px-3 py-2.5 text-sm text-slate-900 placeholder:text-slate-500 outline-none focus:ring-2 focus:ring-primary-500/40 dark:border-slate-800 dark:bg-slate-950/40 dark:text-slate-50 dark:placeholder:text-slate-600"
                      />
                    </div>
                    <div>
                      ...
                      <input
                        ...
                        value={form.tags}
                        onChange={onChange}
                        placeholder="phone, flagship, 2026"
                        className="mt-1 w-full rounded-xl border border-slate-300 bg-slate-50 px-3 py-2.5 text-sm text-slate-900 placeholder:text-slate-500 outline-none focus:ring-2 focus:ring-primary-500/40 dark:border-slate-800 dark:bg-slate-950/40 dark:text-slate-50 dark:placeholder:text-slate-600"
                      />
                      <p className="mt-1 text-[0.65rem] text-slate-500 dark:text-slate-400">
